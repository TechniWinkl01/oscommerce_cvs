<?php
/*
  $Id: smtp.php,v 1.1 2001/06/10 10:16:58 mbs Exp $
  ...
  based on PHPlibrary smtp.php v1.10 2000/06/03 02:29:41
*/

  class smtp_class {
    var $host_name="", $host_port=25, $localhost="", $timeout=0, $error="", $debug=0, $esmtp=1, $esmtp_host="", $esmtp_extensions=array(), $maximum_piped_recipients=100;

    /* private variables - DO NOT ACCESS */

    var $state="Disconnected", $connection=0, $pending_recipients=0;
...
/* class functions */
    ...
    function VerifyResultLines($code,$responses="") {
      if(GetType($responses)!="array") $responses=array();
      Unset($match_code);
      while(($line=$this->GetLine($this->connection))) {
        ...
        $responses[]=strtok("");
        if(!strcmp($match_code,strtok($line," "))) return(1);
      }
      return(-1);
    }
    ...
    function FlushRecipients() {
      if($this->pending_sender) {
        if($this->VerifyResultLines("250")<=0) return(0);
        $this->pending_sender=0;
      }
      for(;$this->pending_recipients;$this->pending_recipients--) {
        if($this->VerifyResultLines(array("250","251"))<=0) return(0);
      }
      return(1);
    }
...
/* Public methods */
    ...
    function Connect() {
      ...
      if(!($this->connection=($this->timeout ? fsockopen($this->host_name,$this->host_port,&$errno,&$error,$this->timeout) : fsockopen($this->host_name,$this->host_port)))) {
        switch($error) {
          case -3: $this->error="-3 socket could not be created";
            return(0);
          case -4: $this->error="-4 dns lookup on hostname \"".$host_name."\" failed";
            return(0);
          case -5: $this->error="-5 connection refused or timed out";
            return(0);
          case -6: $this->error="-6 fdopen() call failed";
            return(0);
          case -7: $this->error="-7 setvbuf() call failed";
            return(0);
          default: $this->error=$error." could not connect to the host \"".$this->host_name."\"";
            return(0);
        }
      } else {
        if(!strcmp($localhost=$this->localhost,"") && !strcmp($localhost=getenv("SERVER_NAME"),"") && !strcmp($localhost=getenv("HOST"),"")) $localhost="localhost";
        $success=0;
        if($this->VerifyResultLines("220")>0) {
          ...
          if(!$success && $this->PutLine("HELO $localhost") && $this->VerifyResultLines("250")>0) $success=1;
        }
        if($success) {
    ...
    function MailFrom($sender) {
      ...
      if(!IsSet($this->esmtp_extensions["PIPELINING"]) && $this->VerifyResultLines("250")<=0) return(0);
      $this->state="SenderSet";
      if(IsSet($this->esmtp_extensions["PIPELINING"]))  $this->pending_sender=1;
    ...
    function SetRecipient($recipient) {
      switch($this->state) {
        case "SenderSet":
        case "RecipientSet": break;
        default: $this->error="connection is not in the recipient setting state";
        return(0);
      }
      $this->error="";
      if(!$this->PutLine("RCPT TO:<$recipient>")) return(0);
      if(IsSet($this->esmtp_extensions["PIPELINING"])) {
        $this->pending_recipients++;
        if($this->pending_recipients>=$this->maximum_piped_recipients) {
          if(!$this->FlushRecipients()) return(0);
        }
      } else {
        if($this->VerifyResultLines(array("250","251"))<=0) return(0);
      }
    ...
    function StartData() {
      ...
      if($this->VerifyResultLines("354")<=0) return(0);
    ...
    function EndSendingData() {
      ...
      $this->error="";
      if(!$this->PutLine("\r\n.") || $this->VerifyResultLines("250")<=0) return(0);
      $this->state="Connected";
      return(1);
    }
    ...
    function ResetConnection() {
      switch($this->state) {
        ...
        case "SendingData": $this->error="can not reset the connection while sending data";
          return(0);
        case "Disconnected": $this->error="can not reset the connection before it is established";
          return(0);
      }
      $this->error="";
      if(!$this->PutLine("RSET") || $this->VerifyResultLines("250")<=0) return(0);
      $this->state="Connected";
      return(1);
    }
    ...
    function Disconnect($quit=1) {
      ...
      $this->error="";
      if(!strcmp($this->state,"Connected") && $quit && (!$this->PutLine("QUIT") || $this->VerifyResultLines("221")<=0)) return(0);
      fclose($this->connection);
      $this->connection=0;
      $this->state="Disconnected";
      return(1);
    ...
    function SendMessage($sender,$recipients,$headers,$body) {
      ...
      for($recipient=0;$recipient<count($recipients);$recipient++) {
        if(!($success=$this->SetRecipient($recipients[$recipient]))) break;
      }
      if($success && ($success=$this->StartData())) {
        for($header_data="",$header=0;$header<count($headers);$header++)
          ...
          if($success) $success=$this->EndSendingData();
          }
        }
        $error=$this->error;
