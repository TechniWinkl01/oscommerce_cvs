<?php
/*
  $Id: mail.php,v 1.1 2001/06/10 10:16:59 mbs Exp $
  ...
  HTML Mime Mail class V1.34 by Richard Heyes <richard.heyes@heyes-computing.net>
  Notes: Based upon mime_mail.class
         by Tobias Ratschiller <tobias@dnet.it> and Sascha Schumann <sascha@schumann.cx>.
         See http://www.heyes-computing.net/scripts/ for full tar/zip if you haven't got one.
*/

  class html_mime_mail{
    var $mime, $html, $body, $do_html, $multipart, $html_text, $html_images, $headers, $parts, $charset, $charsetlist;
...
//  Accessor function to set the content charset. Matt add 2000/10/19
...
// Builds the multipart message from the list ($this->_parts).
    function build_message() {
      if(isset($this->do_html) AND $this->do_html == 1) $multipart=1;
      else $multipart = (count($this->parts)); // yes or no
      $boundary = '=_'.md5(uniqid(time()));

      if ($multipart) {
        $this->headers[] = 'MIME-Version: 1.0';
        if(isset($this->do_html) AND $this->do_html == 1) {
          $this->headers[] = 'Content-Type: multipart/alternative;'.chr(10).chr(9).'boundary="'.$boundary.'"';
        } else {
          $this->headers[] = 'Content-Type: multipart/mixed;'.chr(10).chr(9).'boundary="'.$boundary.'"';
        }
        $this->multipart = "This is a MIME encoded message.\n\n";
      } else {
        $this->headers[] = 'Content-Type: text/plain'.chr(10).chr(9).'Content-Transfer-Encoding: 8bit';
      }
      ...
      // if(isset($this->body) AND $this->body != '') $this->parts[] = array('body' => $this->body, 'name' => '', 'c_type' => 'message/rfc822');
      ...
      if ($multipart) {
        ...
        $this->multipart.= $this->parts[0]['body'];
        $this->mime = $this->multipart;
      }
    }
...
// Use this method to deliver using direct smtp connection. Relies upon Manuel Lemos' smtp mail delivery class available at: http://phpclasses.upperdesign.com void smtp_send( string *Name* of smtp object, string From address, array  To addresses, string Subject, array  Extra headers)
