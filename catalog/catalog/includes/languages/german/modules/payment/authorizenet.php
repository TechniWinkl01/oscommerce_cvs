<?
  define('MODULE_PAYMENT_AUTHORIZENET_TEXT_DESCRIPTION', 'Authorize.net');
  define('MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_NUMBER', 'Kreditkartenr.:');
  define('MODULE_PAYMENT_AUTHORIZENET_TEXT_CREDIT_CARD_EXPIRES', 'Gültig bis:');
  define('MODULE_PAYMENT_AUTHORIZENET_TEXT_JS_CC_NUMBER', '* Die \'Kreditkartenr.\' muß mindestens aus ' . CC_NUMBER_MIN_LENGTH . ' Zahlen bestehen.\n');
  define('MODULE_PAYMENT_AUTHORIZENET_TEXT_ERROR_MESSAGE', 'There has been an error processing you credit card, please try again.');

  define('EMAIL_TEXT_PAYMENT_EXTRA_MESSAGE', 'Ihr Einkauf wurde bereits mit Ihrer Kreditkarte bezahlt.');
?>