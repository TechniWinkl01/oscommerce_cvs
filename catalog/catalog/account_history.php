<?php
/*
  $Id: account_history.php,v 1.60 2003/05/27 17:13:10 hpdl Exp $
  ...
  if (($orders_total = tep_count_customer_orders()) > 0) {
    ...
    while ($history = tep_db_fetch_array($history_query)) {
      ...
      $order = '<table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n" .
               '  <tr>' . "\n" .
               '    <td class="main" width="50%" valign="top"><b>' . TEXT_ORDER_DATE . '</b> ' . tep_date_long($history['date_purchased']) . '<br><b>' . $order_type . '</b> ' . $order_name . '</td>' . "\n" .
               ...
               '    <td class="main" width="20%"><a href="' . tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, 'page=' . $HTTP_GET_VARS['page'] . '&order_id=' . $history['orders_id'], 'SSL') . '">' . tep_image_button('small_view.gif', SMALL_IMAGE_BUTTON_VIEW) . '</a></td>' . "\n" .
               '  </tr>' . "\n" .
               '</table>';
