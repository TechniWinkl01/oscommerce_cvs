...
<? $include_file = DIR_WS_LANGUAGES . $language . '/' . FILENAME_CONTACT_US; include(DIR_WS_INCLUDES . 'include_once.php'); ?>
<? $location = ' : <a href="' . tep_href_link(FILENAME_CONTACT_US, '', 'NONSSL') . '" class="whitelink">' . NAVBAR_TITLE . '</a>'; ?>
<?
  if ($HTTP_GET_VARS['action'] == 'send') {
    tep_mail(STORE_OWNER_EMAIL_ADDRESS, EMAIL_SUBJECT, $HTTP_POST_VARS['enquiry'], "From: " . $HTTP_POST_VARS['name'] . ' <' . $HTTP_POST_VARS['email'] . '>');
...
?>
      ...
      <tr>
        <td align="right"><br><?php echo '<a href="' . tep_href_link(FILENAME_DEFAULT, '', 'NONSSL') . '">' . tep_image_button('button_continue.gif', IMAGE_CONTINUE) . '</a>'; ?>&nbsp;&nbsp;</td>
      </tr>
<?
  } else {
?>
      ...
      <tr>
        <td><form action="<? echo tep_href_link(FILENAME_CONTACT_US, 'action=send', 'NONSSL'); ?>" method="post"><table border="0" width="100%" cellspacing="0" cellpadding="2">
          ...
          <tr>
            <td colspan="2" class="main"><?php echo tep_black_line(); ?></td>
          </tr>
          <tr>
            <td align="right" colspan="2" class="main"><?php echo tep_image_submit('button_continue.gif', IMAGE_CONTINUE); ?>&nbsp;&nbsp;</td>
          </tr>
        </table></form></td>
<?
  }
