<?php
/*
  $Id: product_notification.php,v 1.1 2002/03/10 11:42:27 harley_vb Exp $

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2002 osCommerce

  Released under the GNU General Public License
*/

define('TEXT_COUNT_CUSTOMERS', 'Kunden, welche das Rundschreiben erhalten: %s');
define('TEXT_PRODUCTS', 'Produkte');
define('TEXT_SELECTED_PRODUCTS', 'Produkte ausw&auml;hlen');

define('JS_PLEASE_SELECT_PRODUCTS', 'Bitte wählen Sie einige Produkte aus.');

define('BUTTON_GLOBAL', 'Global');
define('BUTTON_SELECT', '>>>');
define('BUTTON_UNSELECT', '<<<');
define('BUTTON_SUBMIT', 'Weiter');
define('BUTTON_CANCEL', 'Abbrechen');
?>